		...
		lv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				{// 发送获取下一页的任务
					...
					it.putExtra("Task", TaskType.TS_GET_FRIENDS_HOMETIMELINE_MORE);
...
//					HomeActivity.this.startActivityForResult(it, 0);
		...
		MyLog.t("HomeFragment -------------- onCreateVew()");
		return rootView;
	}
...
//		}
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		case 2:// 转发
			...
			if (re_st != null) {
				it.putExtra("currentContent", "//@" + st.getUser().getName() + "：" + st.getText());
			}else{
				it.putExtra("currentContent", "");
			}
			
			startActivity(it);
			// Toast.makeText(getActivity(), "您要转发" + acm.id, Toast.LENGTH_SHORT).show();
			break;
		...
		case 3:// 评论
			...
			// Toast.makeText(getActivity(), "您要评论" + acm.id, Toast.LENGTH_SHORT).show();
			break;
			 
		case 4:// 收藏
			Toast.makeText(getActivity(), "收藏" + acm.id, Toast.LENGTH_SHORT).show();
			break;
		 ...
		 case 5: //查看个人资料
			...
			//Toast.makeText(getActivity(), "查看个人资料" + acm.id, Toast.LENGTH_SHORT).show();
			break;

		 case 6: //复制原微博
			re_st = st.getRetweetedStatus();
			if (re_st != null) {
				content = "@" + re_st.getUser().getName() + "：" 	+ re_st.getText();
				cm = (ClipboardManager) getActivity().getSystemService(getActivity().CLIPBOARD_SERVICE);
	...
	public void refresh(Object... param) {
		...
		case TaskType.TS_GET_FRIENDS_HOMETIMELINE:// 更新首页微博
			...
			if (alls != null) {
				...
				WeiboAdapter wa = new WeiboAdapter(alls, getActivity());
				lv.setAdapter(wa);
			}
		}
	}
